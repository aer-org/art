...
function createSchema(database) {
  ...
  `);
    ...
    // Pipeline stage issues table
    database.exec(`
    CREATE TABLE IF NOT EXISTS pipeline_stage_issues (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      group_folder TEXT NOT NULL,
      stage TEXT NOT NULL,
      tool TEXT NOT NULL,
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS idx_psi_tooluse ON pipeline_stage_issues(tool_use_id);
    CREATE INDEX IF NOT EXISTS idx_psi_stage ON pipeline_stage_issues(group_folder, stage);
  `);
...
// --- Pipeline stage issue accessors ---
export function insertPipelineIssue(issue) {
    db.prepare(`INSERT OR IGNORE INTO pipeline_stage_issues
     (group_folder, stage, tool, tool_use_id, input_preview, input_hash, error_content, assistant_context, turn_index, timestamp)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(issue.groupFolder, issue.stage, issue.tool, issue.toolUseId, issue.inputPreview, issue.inputHash, issue.errorContent, issue.assistantContext ?? null, issue.turnIndex ?? null, issue.timestamp);
}
export function resolvePipelineIssues(inputHash, stage, groupFolder) {
    db.prepare(`UPDATE pipeline_stage_issues SET resolved = 1
     WHERE input_hash = ? AND stage = ? AND group_folder = ? AND resolved = 0`).run(inputHash, stage, groupFolder);
}
export function getPipelineIssues(groupFolder, stage) {
    if (stage) {
        return db
            .prepare(`SELECT * FROM pipeline_stage_issues
         WHERE group_folder = ? AND stage = ?
         ORDER BY timestamp DESC`)
            .all(groupFolder, stage);
    }
    return db
        .prepare(`SELECT * FROM pipeline_stage_issues
       WHERE group_folder = ?
...
export function getUnresolvedIssueCount(groupFolder, stage) {
    const row = db
        .prepare(`SELECT COUNT(*) as count FROM pipeline_stage_issues
       WHERE group_folder = ? AND stage = ? AND resolved = 0`)
        .get(groupFolder, stage);
    return row.count;
}
// --- JSON migration ---
