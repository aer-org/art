...
const DEFAULT_CREDS_PATH = path.join(os.homedir(), '.claude', '.credentials.json');
...
export class OAuthRefresher {
    ...
    static isAvailable(credsPath = DEFAULT_CREDS_PATH) {
        try {
    ...
    async getAccessToken(force = false) {
        ...
        if (!disk)
            throw new Error(`Credentials missing at ${this.credsPath}`);
        this.cache = disk;
    ...
    writeCreds(updated) {
        let full;
        try {
            full = JSON.parse(fs.readFileSync(this.credsPath, 'utf-8'));
        }
        catch {
            full = {};
        }
    ...
    async refreshLocked(force) {
        ...
        try {
            ...
            const disk = this.readCreds();
            if (!disk)
                throw new Error(`Credentials vanished at ${this.credsPath}`);
            if (!force && !this.isExpired(disk)) {
                this.cache = disk;
                return disk.accessToken;
            }
            ...
            this.cache = next;
            logger.info({ expiresAt: new Date(next.expiresAt).toISOString() }, 'OAuth access token refreshed');
            ...
            const req = httpsRequest({
                ...
                res.on('end', () => {
                    ...
                    if (res.statusCode !== 200) {
                        reject(new Error(`OAuth refresh failed (${res.statusCode}): ${text.slice(0, 300)}`));
                        return;
                    }
                    try {
