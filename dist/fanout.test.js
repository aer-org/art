...
import { applyFanoutSubstitutions, assertFanoutDepthAllowed, deriveChildScopeId, loadFanoutTemplate, MAX_FANOUT_RECURSION_DEPTH, parseFanoutPayload, readFanoutDepth, withConcurrency, } from './fanout.js';
describe('parseFanoutPayload', () => {
    it('parses a valid JSON array of flat objects', () => {
        const out = parseFanoutPayload('[{"name":"a","n":1},{"name":"b","flag":true}]', 'fan');
        expect(out).toEqual([
            { name: 'a', n: 1 },
            { name: 'b', flag: true },
        ]);
    ...
    it('rejects invalid JSON', () => {
        expect(() => parseFanoutPayload('{not json', 'fan')).toThrow(/not valid JSON/);
    });
    it('rejects non-array top level', () => {
        expect(() => parseFanoutPayload('{"x":1}', 'fan')).toThrow(/must be a JSON array/);
    });
    it('rejects non-object element', () => {
        expect(() => parseFanoutPayload('[1,2,3]', 'fan')).toThrow(/must be an object/);
    });
    it('rejects nested object/array values (flat-only)', () => {
        expect(() => parseFanoutPayload('[{"x":{"nested":1}}]', 'fan')).toThrow(/must be string \| number \| boolean/);
        expect(() => parseFanoutPayload('[{"x":[1,2]}]', 'fan')).toThrow(/must be string \| number \| boolean/);
    });
});
describe('applyFanoutSubstitutions', () => {
    ...
    it('substitutes {{key}} in allowed fields', () => {
        const out = applyFanoutSubstitutions(template, { name: 'mymod', port: 8080 }, ['prompt'], 'fan');
        expect(out.stages[0].prompt).toBe('Build mymod on port 8080');
    });
    it('leaves missing keys intact', () => {
        const out = applyFanoutSubstitutions(template, { name: 'x' }, ['prompt'], 'fan');
    ...
    it('does not touch fields outside the allowlist', () => {
        ...
        const out = applyFanoutSubstitutions(tmpl, { key: 'src' }, ['prompt'], 'fan');
            ...
            stages: [
                {
                    name: 's',
                    prompt: 'x',
                    mounts: { 'project:{{sub}}': 'rw' },
                    transitions: [{ marker: 'DONE', next: null }],
                },
            ],
        };
        const out = applyFanoutSubstitutions(tmpl, { sub: 'src/mod-a' }, ['mounts'], 'fan');
...
describe('withConcurrency', () => {
    it('runs unbounded when max is undefined', async () => {
        ...
        await withConcurrency(undefined, Array.from({ length: 5 }, () => task()));
        expect(peak).toBe(5);
    });
    it('caps concurrent tasks at max', async () => {
        ...
        await withConcurrency(2, Array.from({ length: 6 }, () => task()));
        expect(peak).toBe(2);
    });
...
describe('fanout depth tracking', () => {
    ...
    afterEach(() => {
        if (prev === undefined)
            ...
            process.env.ART_FANOUT_DEPTH = prev;
    });
    ...
    it('rejects depth past MAX_FANOUT_RECURSION_DEPTH', () => {
        ...
        expect(() => assertFanoutDepthAllowed('s')).toThrow(/recursion depth .* exceeds max/);
    });
});
describe('loadFanoutTemplate', () => {
    ...
    it('rejects template path that escapes groupDir', () => {
        const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'art-fanout-'));
        try {
            expect(() => loadFanoutTemplate(tmpDir, '../escape.json', 'fan')).toThrow(/escapes groupDir/);
        }
        finally {
            fs.rmSync(tmpDir, { recursive: true, force: true });
        }
    ...
    it('rejects missing template file', () => {
        const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'art-fanout-'));
        try {
            expect(() => loadFanoutTemplate(tmpDir, 'missing.json', 'fan')).toThrow(/template file not found/);
        }
        finally {
            fs.rmSync(tmpDir, { recursive: true, force: true });
        }
