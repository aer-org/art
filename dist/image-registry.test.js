...
describe('image-registry', () => {
    ...
    describe('saveImageRegistry', () => {
        it('writes JSON and creates parent dirs', async () => {
            ...
            expect(fs.mkdirSync).toHaveBeenCalledWith(path.dirname('/mock/.config/aer-art/images.json'), { recursive: true });
            expect(fs.writeFileSync).toHaveBeenCalledWith('/mock/.config/aer-art/images.json', JSON.stringify(registry, null, 2) + '\n');
        });
    });
    describe('getImageForStage', () => {
        ...
        it('agent mode, not in registry → returns stageImage as-is', async () => {
            ...
            const { getImageForStage } = await loadModule();
            expect(getImageForStage('some-direct-image:latest')).toBe('some-direct-image:latest');
        });
    });
});
//# sourceMappingURL=image-registry.test.js.map