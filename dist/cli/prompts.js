...
import { getPromptById, getPromptDbPath, listPrompts, promptPreview, queryPrompts, resolvePromptIds, } from '../prompt-store.js';
function readPipelineFile(inputPath) {
    ...
    const pipelinePath = stats?.isDirectory() || !stats
...
export async function promptsCli(args) {
    ...
    switch (subcommand) {
        ...
        case 'get': {
            const id = rest[0];
            if (!id)
                throw new Error('Usage: art prompts get <db_id>');
            const prompt = getPromptById(id, dbPath);
            if (!prompt)
                throw new Error(`Prompt "${id}" not found in ${dbPath}`);
            console.log(`id: ${prompt.id}`);
            if (prompt.title)
                console.log(`title: ${prompt.title}`);
            if (prompt.updated_at)
                console.log(`updated_at: ${prompt.updated_at}`);
        ...
        case 'query': {
            ...
            if (!term)
                throw new Error('Usage: art prompts query <text>');
