...
export async function run(targetDir, opts) {
    ...
    if (pipelineConfig) {
        ...
        const allStages = [...pipelineConfig.stages];
        const visitedTemplates = new Set();
        function collectTemplateStages(stages) {
        ...
        const checked = new Set(); // dedupe by image name
        for (const stage of allStages) {
            ...
            if (fs.existsSync(dockerfilePath)) {
                ...
                console.log(`\n  Building ${imageTag}...`);
                execSync(`${rt.bin} build -t ${imageTag} -f ${dockerfilePath} ${artDir}`, { stdio: 'inherit', timeout: 600000 });
                registry[imageName] = {
                    image: imageTag,
                    hasAgent: true,
                    contentHash: hash,
