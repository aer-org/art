...
import { contentHash, saveBundleMeta, extractAgentPrompts, } from '../bundle.js';
...
export async function pull(args) {
    ...
    const { stripped, agents: inlineAgents } = extractAgentPrompts(bundle.pipeline.content);
    ...
    if (templateNames.length > 0) {
        ...
        for (const [name, tpl] of Object.entries(bundle.templates)) {
            ...
            const { stripped: tplStripped, agents: tplAgents } = extractAgentPrompts(tpl.content);
    ...
    if (allAgents.size > 0)
        console.log(`  agents/ (${allAgents.size} agent${allAgents.size > 1 ? 's' : ''})`);
    if (templateNames.length > 0)
        console.log(`  templates/ (${templateNames.length} template${templateNames.length > 1 ? 's' : ''})`);
    if (dockerfileNames.length > 0)
        console.log(`  dockerfiles/ (${dockerfileNames.length} dockerfile${dockerfileNames.length > 1 ? 's' : ''})`);
}
