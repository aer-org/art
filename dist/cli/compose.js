...
Use Korean if the project contains Korean documentation, otherwise use English.`;
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (method === 'GET' && parsed.pathname === '/api/runs/current') {
            ...
            const running = listRunManifests(artDir).find((m) => m.status === 'running');
            res.writeHead(200, { 'Content-Type': 'application/json' });
...
export async function compose(targetDir, opts) {
    ...
    if (!fs.existsSync(artDir)) {
        ...
        const answer = await new Promise((resolve) => rl.question(`No ${ART_DIR_NAME}/ found. Initialize project? (y/N): `, resolve));
        rl.close();
    ...
    if (!fs.existsSync(pipelineFile)) {
        console.error(`No PIPELINE.json found in ${artDir}. Run 'art compose .' to re-initialize.`);
        process.exit(1);
    }
