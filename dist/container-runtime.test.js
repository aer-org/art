...
const { mockExecSync, mockReadFileSync, mockExistsSync, mockWriteFileSync, mockMkdirSync, } = vi.hoisted(() => ({
    mockExecSync: vi.fn(),
...
import { readonlyMountArgs, writableMountArgs, stopContainer, hostGatewayArgs, ensureContainerRuntimeRunning, cleanupOrphans, cleanupRunContainers, getRuntime, getRuntimeBin, getHostGateway, getProxyBindHost, getRuntimeCapabilities, initRuntime, _resetRuntime, _setRuntime, } from './container-runtime.js';
import { logger } from './logger.js';
function makeRuntime(kind, overrides) {
    const defaults = {
        docker: {
            kind: 'docker',
            bin: 'docker',
...
describe('stopContainer', () => {
    it('uses docker by default', () => {
        expect(stopContainer('aer-art-test-123')).toBe('docker stop aer-art-test-123');
    });
    it('uses the runtime bin when set', () => {
        _setRuntime(makeRuntime('podman'));
        expect(stopContainer('aer-art-test-123')).toBe('podman stop aer-art-test-123');
    });
});
// --- hostGatewayArgs ---
...
describe('cleanupOrphans', () => {
    it('stops orphaned containers with docker/podman', () => {
        _setRuntime(makeRuntime('docker'));
        mockExecSync.mockReturnValueOnce('aer-art-group1-111\naer-art-group2-222\n');
        mockExecSync.mockReturnValue('');
        cleanupOrphans();
        expect(mockExecSync).toHaveBeenCalledTimes(3);
        expect(mockExecSync).toHaveBeenNthCalledWith(2, 'docker stop aer-art-group1-111', { stdio: 'pipe' });
        expect(mockExecSync).toHaveBeenNthCalledWith(3, 'docker stop aer-art-group2-222', { stdio: 'pipe' });
        expect(logger.info).toHaveBeenCalledWith({ count: 2, names: ['aer-art-group1-111', 'aer-art-group2-222'] }, 'Stopped orphaned containers');
    });
    ...
    it('skips orphan cleanup for udocker (no ps --filter support)', () => {
        ...
        expect(logger.debug).toHaveBeenCalledWith({ runtime: 'udocker' }, 'Runtime does not support ps --filter, skipping orphan cleanup');
    });
    ...
    it('continues stopping remaining containers when one stop fails', () => {
        ...
        cleanupOrphans();
        expect(mockExecSync).toHaveBeenCalledTimes(3);
        expect(logger.info).toHaveBeenCalledWith({ count: 2, names: ['aer-art-a-1', 'aer-art-b-2'] }, 'Stopped orphaned containers');
    });
});
...
describe('cleanupRunContainers', () => {
    it('stops containers matching the run ID label', () => {
        ...
        expect(mockExecSync).toHaveBeenCalledTimes(3);
        expect(mockExecSync).toHaveBeenNthCalledWith(1, "docker ps --filter label=art-run-id=run-123-abc --format '{{.Names}}'", { stdio: ['pipe', 'pipe', 'pipe'], encoding: 'utf-8' });
        expect(mockExecSync).toHaveBeenNthCalledWith(2, 'docker stop aer-art-build-111', { stdio: 'pipe' });
        expect(logger.info).toHaveBeenCalledWith({ count: 2, runId: 'run-123-abc' }, 'Cleaned up orphaned run containers');
    });
...
describe('initRuntime', () => {
    ...
    it('auto-detects runtimes in priority order (docker > podman > udocker)', async () => {
        ...
        mockExecSync.mockImplementation((cmd) => {
            ...
            if (cmd === 'udocker version')
                return 'udocker 1.3.1';
            return '';
        });
        mockExistsSync.mockReturnValue(false);
        mockMkdirSync.mockReturnValue(undefined);
