...
import { runContainerAgent, spawnSubAgentContainer, writeGroupsSnapshot, writeTasksSnapshot, } from './container-runner.js';
import { loadAgentTeamConfig, loadPipelineConfig, PipelineRunner, } from './pipeline-runner.js';
import { cleanupOrphans, ensureContainerRuntimeRunning, getProxyBindHost, initRuntime, } from './container-runtime.js';
...
async function processGroupMessages(chatJid) {
    ...
    const typingInterval = setInterval(() => {
        if (Date.now() - lastResultTime > TYPING_IDLE_MS) {
            logger.debug({ group: group.name }, 'Typing idle timeout, stopping keepalive');
    ...
    if (prompt.includes('파이프라인 초기화')) {
        ...
        if (teamConfig) {
            ...
            const agents = targetAgent
                ? teamConfig.agents.filter((a) => a.name === targetAgent || a.folder === targetAgent)
                : teamConfig.agents;
            for (const agent of agents) {
                const stateFile = path.join(parentGroupDir, agent.folder, 'PIPELINE_STATE.json');
                try {
                ...
                await channel.sendMessage(chatJid, `🗑️ ${agent.name} 파이프라인 초기화 완료`);
            }
        }
        else {
    ...
    if (prompt.includes('구현 시작')) {
        ...
        if (teamConfig) {
            const parentGroupDir = resolveGroupFolderPath(group.folder);
            logger.info({ group: group.name, agentCount: teamConfig.agents.length }, 'Agent team pipeline mode detected');
            const results = await Promise.all(teamConfig.agents.map(async (agent) => {
                ...
                if (!pipelineConfig) {
                    await channel.sendMessage(chatJid, `⚠️ ${agent.name}: PIPELINE.json 없음`);
                    return 'error';
                }
...
async function ensureContainerSystemRunning() {
    await initRuntime();
                    ...
                    if (output.result) {
                        ...
                        if (channel) {
                            ...
                            const text = raw
                                ...
                                .trim();
                            if (text)
                                await channel.sendMessage(request.chatJid, text);
                        }
                    }
                },
            }).catch((err) => {
                logger.error({ agentId: request.agentId, err }, 'Sub-agent spawn failed');
            });
        },
    });
