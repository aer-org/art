...
export async function runPipeline(opts) {
    ...
    if (stagesWithRefs.length > 0) {
        const { RegistryClient, loadCredentials, canonicalImageTag } = await import('./registry-client.js');
        const creds = loadCredentials();
        if (!creds) {
            console.error("Pipeline references registry agents but no credentials found. Run 'art login'.");
            proxyServer?.close();
            codexAuthProxyServer?.close();
            process.exit(1);
        }
        ...
        for (const stage of stagesWithRefs) {
            ...
            try {
                ...
                if (!stage.prompt)
                    stage.prompt = version.system_prompt;
                ...
                if (version.dockerfile_hash && version.dockerfile_image_name) {
                    const df = await client.fetchDockerfileVersion(version.dockerfile_hash);
                ...
                console.log(`Resolved ${stage.name}: ${ref} → ${hash.slice(0, 19)}…${stage.image ? ` (image: ${stage.image})` : ''}`);
            }
            catch (e) {
                console.error(`Failed to resolve '${ref}' for stage '${stage.name}': ${e.message}`);
                proxyServer?.close();
                codexAuthProxyServer?.close();
                process.exit(1);
            }
