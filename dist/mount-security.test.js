...
function writeAllowlist(dir, opts = {}) {
    const content = {
...
describe('mount-security', () => {
    let tmpDir;
    let loadMountAllowlist;
    let validateMount;
    let validateAdditionalMounts;
    let generateAllowlistTemplate;
    beforeEach(async () => {
    ...
    describe('validateMount — container path validation', () => {
        ...
        it('rejects container path containing ".."', () => {
            const result = validateMount({ hostPath: path.join(tmpDir, 'data'), containerPath: '../escape' }, true);
        ...
        it('falls back to hostPath basename when containerPath is empty string', () => {
            const result = validateMount({ hostPath: path.join(tmpDir, 'data'), containerPath: '' }, true);
        ...
        it('rejects container path with embedded ".." component', () => {
            const result = validateMount({ hostPath: path.join(tmpDir, 'data'), containerPath: 'a/../b' }, true);
            expect(result.allowed).toBe(false);
            expect(result.reason).toContain('..');
        });
    });
    ...
    describe('validateMount — blocked patterns', () => {
        ...
        it('rejects path matching credentials in a component', () => {
            ...
            const result = validateMount({ hostPath: path.join(tmpDir, 'credentials') }, true);
            expect(result.allowed).toBe(false);
            expect(result.reason).toContain('credentials');
        });
        it('rejects path matching private_key', () => {
            fs.writeFileSync(path.join(tmpDir, 'private_key'), '');
            const result = validateMount({ hostPath: path.join(tmpDir, 'private_key') }, true);
            expect(result.allowed).toBe(false);
        ...
        it('rejects path containing blocked pattern in parent directory', () => {
            ...
            const result = validateMount({ hostPath: path.join(tmpDir, '.ssh', 'authorized_keys') }, true);
            expect(result.allowed).toBe(false);
            expect(result.reason).toContain('.ssh');
        });
        it('rejects path where component contains blocked pattern as substring', () => {
            fs.mkdirSync(path.join(tmpDir, 'my_credentials_backup'));
            const result = validateMount({ hostPath: path.join(tmpDir, 'my_credentials_backup') }, true);
            expect(result.allowed).toBe(false);
            expect(result.reason).toContain('credentials');
        });
        it('rejects custom blocked patterns from allowlist', () => {
            ...
            const result = validateMount({ hostPath: path.join(tmpDir, 'super_secret') }, true);
            expect(result.allowed).toBe(false);
    ...
    describe('validateMount — symlink traversal', () => {
        ...
        it('rejects symlink that resolves to a blocked path', () => {
            ...
            const result = validateMount({ hostPath: path.join(tmpDir, 'innocent'), containerPath: 'data' }, true);
            expect(result.allowed).toBe(false);
            expect(result.reason).toContain('.ssh');
        });
        it('rejects symlink that resolves outside allowed root', () => {
            ...
            try {
                ...
                const result = validateMount({ hostPath: path.join(tmpDir, 'escape'), containerPath: 'data' }, true);
                expect(result.allowed).toBe(false);
        ...
        it('rejects chained symlinks (A → B → blocked path)', () => {
            ...
            const result = validateMount({ hostPath: path.join(tmpDir, 'link-a'), containerPath: 'data' }, true);
            expect(result.allowed).toBe(false);
            expect(result.reason).toContain('.ssh');
        });
        it('allows symlink that resolves to a safe path under allowed root', () => {
            ...
            const result = validateMount({ hostPath: path.join(tmpDir, 'link-safe'), containerPath: 'data' }, true);
            expect(result.allowed).toBe(true);
        ...
        it('rejects broken symlink (target does not exist)', () => {
            ...
            const result = validateMount({ hostPath: path.join(tmpDir, 'broken-link'), containerPath: 'data' }, true);
            expect(result.allowed).toBe(false);
    ...
    it('rejects mounts where host path does not exist', () => {
        writeAllowlist(tmpDir);
        const result = validateMount({ hostPath: path.join(tmpDir, 'nonexistent'), containerPath: 'data' }, true);
        expect(result.allowed).toBe(false);
        expect(result.reason).toContain('does not exist');
    });
    ...
    describe('validateMount — allowed root', () => {
        it('rejects mounts not under any allowed root', () => {
            ...
            try {
                const result = validateMount({ hostPath: outsideDir, containerPath: 'data' }, true);
                expect(result.allowed).toBe(false);
                expect(result.reason).toContain('not under any allowed root');
            }
            finally {
        ...
        it('allows valid mount under an allowed root', () => {
            ...
            const result = validateMount({ hostPath: projectDir, containerPath: 'proj' }, true);
            expect(result.allowed).toBe(true);
    ...
    describe('validateMount — readonly enforcement', () => {
        ...
        it('allows read-write when requested, root allows it, and isMain', () => {
            writeAllowlist(tmpDir);
            const result = validateMount({ hostPath: dataDir, containerPath: 'data', readonly: false }, true);
            expect(result.allowed).toBe(true);
            expect(result.effectiveReadonly).toBe(false);
        });
        it('forces readonly for non-main when nonMainReadOnly is true', () => {
            writeAllowlist(tmpDir, { nonMainReadOnly: true });
            const result = validateMount({ hostPath: dataDir, containerPath: 'data', readonly: false }, false);
            expect(result.allowed).toBe(true);
            expect(result.effectiveReadonly).toBe(true);
        });
    ...
    describe('validateMount — container path derivation', () => {
        ...
        it('uses explicit containerPath when provided', () => {
            ...
            fs.mkdirSync(dir);
            const result = validateMount({ hostPath: dir, containerPath: 'custom-name' }, true);
            expect(result.allowed).toBe(true);
    ...
    describe('validateAdditionalMounts', () => {
        ...
        it('prefixes containerPath with /workspace/extra/', () => {
            ...
            const dir = path.join(tmpDir, 'data');
            fs.mkdirSync(dir);
            const result = validateAdditionalMounts([{ hostPath: dir, containerPath: 'mydata' }], 'test-group', true);
        ...
        it('returns empty array when no mounts are valid', () => {
            ...
            const result = validateAdditionalMounts([{ hostPath: '/some/path', containerPath: 'data' }], 'test-group', true);
        ...
        it('propagates effective readonly status to output', () => {
            ...
            const dir = path.join(tmpDir, 'data');
            fs.mkdirSync(dir);
            const result = validateAdditionalMounts([{ hostPath: dir, containerPath: 'data', readonly: false }], 'worker-group', false);
            expect(result).toHaveLength(1);
    ...
    describe('default blocked patterns', () => {
        ...
        it.each(ALL_DEFAULT_BLOCKED)('blocks mount containing default pattern: %s', (pattern) => {
            writeAllowlist(tmpDir);
            // Create the actual path on disk
            const target = path.join(tmpDir, pattern);
            fs.mkdirSync(target, { recursive: true });
            const result = validateMount({ hostPath: target, containerPath: 'test' }, true);
            expect(result.allowed).toBe(false);
