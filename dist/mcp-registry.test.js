...
import { loadMcpRegistry, resolveStageMcpServers, validateStageMcpAccess, } from './mcp-registry.js';
describe('mcp-registry', () => {
    ...
    it('loads stdio and http registry entries', () => {
        ...
        expect(registry['sqlite.read']).toEqual(expect.objectContaining({
            name: 'sqlite_read',
            transport: 'http',
            url: 'http://${ART_HOST_GATEWAY}:4318/mcp',
        }));
        expect(registry['sqlite.write']).toEqual(expect.objectContaining({
            name: 'sqlite_write',
            transport: 'stdio',
            command: 'node',
        }));
    ...
    it('rejects duplicate server names within one stage', () => {
        ...
        expect(() => validateStageMcpAccess(['first', 'second'])).toThrow(/same server name/);
    });
});
