...
import { createTask, deleteTask, getTaskById, insertPipelineIssue, resolvePipelineIssues, updateTask, } from './db.js';
...
deps) {
    ...
    switch (data.type) {
        ...
        case 'refresh_groups':
            ...
            break;
        case 'spawn_agent':
            if (data.prompt && data.agentId && data.name && data.chatJid) {
                deps.spawnSubAgent({
                    agentId: data.agentId,
                    prompt: data.prompt,
                    name: data.name,
                    parentFolder: sourceGroup,
                    chatJid: data.chatJid,
                    isMain,
                    mounts: data.mounts || [],
                    systemPrompt: data.systemPrompt,
                });
                logger.info({ agentId: data.agentId, name: data.name, sourceGroup }, 'Sub-agent spawn requested via IPC');
            }
            else {
                logger.warn({ data }, 'Invalid spawn_agent request - missing required fields');
            }
            break;
        case 'report_issue':
            if (data.stage &&
                ...
                insertPipelineIssue({
                    ...
                    inputPreview: data.input || '',
                    inputHash: data.inputHash,
                    errorContent: data.errorContent,
                    assistantContext: data.assistantContext || undefined,
                    turnIndex: data.turnIndex || undefined,
                    timestamp: new Date().toISOString(),
                });
                logger.info({ stage: data.stage, tool: data.tool, sourceGroup }, 'Pipeline issue reported via IPC');
            }
            break;
        case 'resolve_issue':
            if (data.inputHash && data.stage) {
                resolvePipelineIssues(data.inputHash, data.stage, sourceGroup);
                logger.info({ stage: data.stage, inputHash: data.inputHash, sourceGroup }, 'Pipeline issue resolved via IPC');
            }
            break;
        case 'register_group':
