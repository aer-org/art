...
Adds Parallel AI MCP integration to AerArt for advanced web research capabilities.
...
- **Non-blocking Design** - Uses AerArt scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. AerArt already set up and running
...
Add Parallel AI MCP servers after the aer-art server:
```typescript
const mcpServers: Record<string, any> = {
  aer-art: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__aer-art__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__aer-art__send_message
   4. Use mcp__aer-art__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo aer-art-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep aer-art  # macOS
...
- Check task runs: `tail -f logs/aer-art.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.aer-art` (macOS) or `systemctl --user restart aer-art` (Linux)
