...
This skill adds Telegram support to AerArt, then walks through interactive setup.
...
```bash
git remote add telegram https://github.com/qwibitai/aer-art-telegram.git
```
...
- Check bot is started: `tail -f logs/aer-art.log`
...
6. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.aer-art` (macOS) or `npm run build && systemctl --user restart aer-art` (Linux)
