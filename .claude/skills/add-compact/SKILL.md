...
> **Note:** `upstream` is the remote pointing to `qwibitai/aer-art`. If using a different remote name, substitute accordingly.
...
1. Start AerArt in dev mode: `npm run dev`
...
```bash
git clone <your-fork> /tmp/aer-art-test
cd /tmp/aer-art-test
...
- **No config file.** AerArt's philosophy is customization through code changes, not configuration sprawl.
