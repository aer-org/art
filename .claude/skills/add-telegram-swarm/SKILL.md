...
1. *Share progress in the group* via `mcp__aer-art__send_message` with a `sender` parameter matching their exact role/character name (e.g., `sender: "Marine Biologist"` or `sender: "Alexander Hamilton"`). This makes their messages appear from a dedicated bot in the Telegram group.
...
Also add `TELEGRAM_BOT_POOL` to the launchd plist (`~/Library/LaunchAgents/com.aer-art.plist`) in the `EnvironmentVariables` dict if using launchd.
...
# macOS:
launchctl unload ~/Library/LaunchAgents/com.aer-art.plist
launchctl load ~/Library/LaunchAgents/com.aer-art.plist
# Linux:
# systemctl --user restart aer-art
```
...
>
> Check logs: `tail -f logs/aer-art.log | grep -i pool`
...
2. Check pool initialized: `grep "Pool bot" logs/aer-art.log`
...
8. Rebuild: `npm run build && ./container/build.sh && launchctl unload ~/Library/LaunchAgents/com.aer-art.plist && launchctl load ~/Library/LaunchAgents/com.aer-art.plist` (macOS) or `npm run build && ./container/build.sh && systemctl --user restart aer-art` (Linux)
