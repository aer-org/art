---
name: x-integration
description: X (Twitter) integration for AerArt. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** AerArt v1.0.0. Directory structure may change in future versions.
...
1. **AerArt is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep aer-art (macOS) or systemctl --user status aer-art (Linux)
```
...
| `AER_ART_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/aer-art.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into AerArt, make the following modifications:
...
All paths below are relative to project root (`AER_ART_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep aer-art  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status aer-art
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/aer-art.log | tail -20

# Script errors
grep -i "error\|failed" logs/aer-art.log | tail -20
```
...
docker run aer-art-agent ls -la /app/src/skills/
```
