...
import { OAuthRefresher } from '../../src/oauth-refresh.js';
...
class TestRefresher extends OAuthRefresher {
  httpCalls = 0;
  tokenUrl: string;
  constructor(credsPath: string, tokenUrl: string) {
    super({ credsPath });
    this.tokenUrl = tokenUrl;
  }
  // Override the private http path via any-cast; we only care about behavior.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  protected httpRefresh(current: any): Promise<any> {
    this.httpCalls++;
...
describe('OAuthRefresher', () => {
  ...
  function writeCreds(expiresInMs: number): void {
    fs.writeFileSync(
      credsPath,
      JSON.stringify({
        claudeAiOauth: {
