...
export interface RuntimeCapabilities {
  canBuild: boolean;
  supportsAutoRemove: boolean; // --rm
  supportsNaming: boolean; // --name
  supportsAddHost: boolean; // --add-host
  supportsDevices: boolean; // --device (default mode)
  supportsDeviceCgroupRule: boolean; // --device-cgroup-rule
  supportsPsFilter: boolean; // ps --filter
  supportsUser: boolean; // --user
  supportsStdin: boolean; // -i (stdin piping)
}
...
export interface RuntimeConfig {
  ...
  hostGateway: string; // 'host.docker.internal' | 'host.containers.internal' | 'localhost'
  bridgeInterface: string | null; // 'docker0' | 'podman0' | null
  selinux: boolean; // mount suffix :z needed
  rootless: boolean; // podman rootless mode
}
...
const PODMAN_CAPABILITIES: RuntimeCapabilities = {
  canBuild: true,
  supportsAutoRemove: true,
  supportsNaming: true,
  supportsAddHost: true,
...
const UDOCKER_CAPABILITIES: RuntimeCapabilities = {
  canBuild: false,
...
function isDockerActuallyPodman(): boolean {
  try {
...
function detectBridgeInterface(kind: RuntimeKind): string | null {
  ...
  if (kind === 'podman') {
    if (ifaces['podman0']) return 'podman0';
    if (ifaces['cni-podman0']) return 'cni-podman0';
    return null;
  }
  return null; // udocker has no bridge
}

function detectProxyBindHost(rt: RuntimeConfig): string {
...
function buildRuntimeConfig(kind: RuntimeKind, bin: string): RuntimeConfig {
...
// ---------------------------------------------------------------------------
// Saved runtime config (persistent across restarts)
// ---------------------------------------------------------------------------

interface SavedRuntimeChoice {
  runtime: RuntimeKind;
  confirmedAt: string;
}

function loadSavedRuntime(): SavedRuntimeChoice | null {
  try {
    const raw = fs.readFileSync(RUNTIME_CONFIG_PATH, 'utf-8');
    return JSON.parse(raw) as SavedRuntimeChoice;
  } catch {
    return null;
  }
}

function saveRuntimeChoice(kind: RuntimeKind): void {
  const dir = path.dirname(RUNTIME_CONFIG_PATH);
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(
    RUNTIME_CONFIG_PATH,
    JSON.stringify(
      { runtime: kind, confirmedAt: new Date().toISOString() },
      null,
      2,
    ) + '\n',
  );
...
function detectAvailableRuntimes(): DetectedRuntime[] {
...
function udockerWarnings(): string {
  return [
...
async function detectAndConfirmRuntime(): Promise<RuntimeConfig> {
  ...
  if (available.length === 0) {
    ...
    console.error(
      '\u2551  3. udocker: https://indigo-dc.github.io/udocker/installation  \u2551',
    );
    console.error(
      '\u255a\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u255d\n',
    );
  ...
  for (const candidate of available) {
    ...
    if (accepted) {
      ...
      logger.info(
        { runtime: candidate.kind, bin: candidate.bin },
        'Container runtime confirmed and saved',
      );
...
export async function initRuntime(): Promise<RuntimeConfig> {
  ...
  const envRuntime = process.env.CONTAINER_RUNTIME as RuntimeKind | undefined;
  if (envRuntime && ['docker', 'podman', 'udocker'].includes(envRuntime)) {
    const bin =
      envRuntime === 'podman' && commandExists('podman')
        ? 'podman'
    ...
    logger.info(
      { runtime: envRuntime, bin, source: 'env' },
      'Container runtime set via CONTAINER_RUNTIME env var',
    );
  ...
  if (saved) {
    const bin =
      saved.runtime === 'podman' && commandExists('podman')
        ? 'podman'
    ...
    logger.warn(
      { runtime: saved.runtime },
      'Saved container runtime no longer available, re-detecting',
    );
  }
...
export function getRuntime(): RuntimeConfig {
  if (!cachedRuntime) {
    throw new Error(
      'Container runtime not initialized. Call initRuntime() at startup.',
    );
  }
  return cachedRuntime;
}
 ...
 */
export function ensureImage(image: string): string {
  const rt = getRuntime();
 ...
 */
export function downloadAndLoadUdockerImage(bin: string): string {
 ...
 */
export function prepareContainer(image: string, name: string): void {
  const rt = getRuntime();
  if (rt.kind !== 'udocker') return;
...
export function hostGatewayArgs(): string[] {
  ...
  if (!rt.capabilities.supportsAddHost) return [];
...
export function ensureContainerRuntimeRunning(): void {
  ...
  try {
    ...
    console.error(
      `\u2551  FATAL: ${rt.kind} failed to start                                ${rt.kind === 'docker' ? '  ' : ''}\u2551`,
    );
    console.error(
      '\u2551                                                                \u2551',
    );
    console.error(
      `\u2551  Run: ${rt.bin} info                                             ${' '.repeat(Math.max(0, 6 - rt.bin.length))}\u2551`,
    );
    console.error(
      '\u255a\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u255d\n',
    );
...
export function cleanupOrphans(): void {
  ...
  try {
    const output = execSync(
      `${rt.bin} ps --filter name=aer-art- --format '{{.Names}}'`,
...
export function cleanupRunContainers(runId: string): void {
  const rt = getRuntime();

  if (!rt.capabilities.supportsPsFilter) return;

  try {
    const output = execSync(
      `${rt.bin} ps --filter label=art-run-id=${runId} --format '{{.Names}}'`,
      { stdio: ['pipe', 'pipe', 'pipe'], encoding: 'utf-8' },
    );
...
export function _setRuntime(rt: RuntimeConfig): void {
  cachedRuntime = rt;
}
