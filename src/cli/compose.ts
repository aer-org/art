...
export async function startEditorServer(
  ...
  const runSseClients: Set<http.ServerResponse> = new Set();
...
Use Korean if the project contains Korean documentation, otherwise use English.`;
  ...
  function relayAgentOutput(chunk: string): void {
    ...
    while (cursor < parseBuffer.length) {
      ...
      let nearestIdx = -1;
      let markerType: 'output' | 'tool' = 'output';
 ...
 */
async function runHeadlessCompose(
  artDir: string,
  mode: 'single' | 'init',
  projectDir: string,
): Promise<void> {
...
export async function compose(
  ...
  if (!fs.existsSync(artDir)) {
    if (headless) {
      ...
      const { scaffoldArtDir, ensureContainerImage } =
