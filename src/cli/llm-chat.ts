...
function buildTree(projectDir: string, files: string[]): string {
  const lines: string[] = [];
...
export function scanCodebase(projectDir: string): string {
  const BUDGET = 40_000;
  const parts: string[] = [];
  let charCount = 0;
...
function analysisSystemPrompt(codebaseContext: string): string {
...
function directionSystemPrompt(
  analysisResult: string,
...
function sseWrite(res: ServerResponse, event: Record<string, unknown>): void {
...
function getSession(id: string): ConversationState | undefined {
  return sessions.get(id);
}

function setSession(id: string, state: ConversationState): void {
...
export function advancePhase(
  ...
  if (state.phase === 'direction') {
    ...
    const metricsMatch = directionContent.match(
      /(?:metrics|메트릭|성공 기준|success criteria)[:\s]*([\s\S]*?)(?:\n##|\n\d+\.|$)/i,
    );
