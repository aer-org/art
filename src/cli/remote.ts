...
export async function remote(args: string[]): Promise<void> {
  ...
  switch (subcommand) {
    case 'add': {
      ...
      if (config.remotes[name]) {
        console.error(
          `Remote "${name}" already exists. Remove it first with "art remote remove ${name}".`,
        );
        process.exit(1);
      }
      ...
      if (isFirst) console.log(`  (set as default)`);
      break;
    }
