...
const DEFAULT_PIPELINE: PipelinePreset = {
  label: 'Full research loop',
  description: 'Build → Test → Review → History',
  templateNames: ['build', 'test', 'review', 'history'],
};

export async function onboard(targetDir: string): Promise<void> {
  ...
  const description = await ask(
    'What are you building? (one sentence is fine, press Enter to skip): ',
  );
  if (description.trim()) {
    fs.writeFileSync(
...
function buildPresetStages(preset: PipelinePreset): Stage[] {
...
async function buildCustomStages(
  ...
  while (addMore) {
    ...
    if (num >= 1 && num <= templates.length) {
      ...
      const customPrompt = await ask(
        `  Custom prompt? (Enter to use default): `,
      );
      stages.push({
        name: template.name,
