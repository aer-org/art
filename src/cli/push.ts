...
export async function push(args: string[]): Promise<void> {
  ...
  if (!meta) {
    console.error(
      `No .art-bundle.json found in ${dir}. Pull a pipeline first with "art pull".`,
    );
    process.exit(1);
  }
  ...
  if (counts.pipelines > 0) parts.push(`${counts.pipelines} pipeline${counts.pipelines > 1 ? 's' : ''}`);
  if (counts.templates > 0) parts.push(`${counts.templates} template${counts.templates > 1 ? 's' : ''}`);
  if (counts.dockerfiles > 0) parts.push(`${counts.dockerfiles} dockerfile${counts.dockerfiles > 1 ? 's' : ''}`);
