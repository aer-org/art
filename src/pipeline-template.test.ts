...
function makeTmpGroupDir(): string {
...
function writeTemplate(groupDir: string, name: string, body: unknown): void {
  fs.writeFileSync(
    path.join(groupDir, 'templates', `${name}.json`),
    JSON.stringify(body),
  );
}
...
describe('resolveTemplatePath', () => {
  ...
  it('rejects absolute-like names', () => {
    expect(() => resolveTemplatePath('/tmp/group', '/etc/passwd')).toThrow(
      /match/,
    );
  });
...
describe('validatePipelineTemplate', () => {
  ...
  it('rejects empty stages', () => {
    expect(() => validatePipelineTemplate({ stages: [] }, 'tpl')).toThrow(
      /non-empty array/,
    );
  });
  ...
  it('rejects count without template', () => {
    ...
    ).toThrow(/count.*requires.*template/);
  });
  ...
  it('rejects both next (string) and template', () => {
    ...
    ).toThrow(/either "next" or "template"/);
  });
  ...
  it('rejects external next references (scope-local only)', () => {
    ...
    ).toThrow(/must reference a stage inside this template/);
  });
});
