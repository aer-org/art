...
export function validateProjectMounts(
  ...
  if (subEntries.length === 0) return { valid: true, errors: [] };
  ...
  if (rootPolicy === null) {
    for (const entry of subEntries) {
      if (entry.policy === 'ro' || entry.policy === 'rw') {
        errors.push(
          `Cannot enable "project:${entry.path}" when project root is disabled`,
        );
      }
    }
  }
  ...
  for (const entry of subEntries) {
    if (entry.policy !== 'ro' && entry.policy !== 'rw') continue;
    ...
    for (let i = 1; i < parts.length; i++) {
      ...
      if (mounts[ancestorKey] === null) {
        errors.push(
          `Cannot enable "project:${entry.path}" when "project:${ancestorPath}" is disabled`,
        );
        break;
      }
    }
