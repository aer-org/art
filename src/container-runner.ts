...
function udockerCreateContainer(
  bin: string,
  image: string,
...
function udockerRemoveContainer(bin: string, containerName: string): void {
  try {
...
export function buildContainerArgs(
  ...
  if (rt.capabilities.supportsStdin) args.push('-i');
  ...
  for (const device of devices) {
    if (device === '/dev/bus/usb') {
      if (!rt.capabilities.supportsDeviceCgroupRule) {
        logger.warn(
          { device, runtime: rt.kind },
          'USB device passthrough not supported, skipping',
        );
        continue;
      }
    ...
    if (!rt.capabilities.supportsDevices) {
      logger.warn(
        { device, runtime: rt.kind },
        'Device passthrough not supported, skipping',
      );
      continue;
    }
...
export async function runContainerAgent(
  ...
  return new Promise((resolve) => {
    ...
    if (rt.kind === 'udocker') {
      udockerCreateContainer(
        rt.bin,
        resolveLocalImageName(image),
        containerName,
      );
    }
    ...
    const killOnTimeout = () => {
      ...
      if (rt.capabilities.supportsNaming) {
        ...
        setTimeout(() => {
          if (!exited) container.kill('SIGKILL');
        }, 15000);
      }
    };
 ...
 */
export async function spawnSubAgentContainer(opts: {
  agentId: string;
  prompt: string;
  name: string;
  parentFolder: string;
  ...
  const subFolder = `${opts.parentFolder}__sub_${opts.name.replace(/[^a-zA-Z0-9-]/g, '-')}`;

  // Write custom system prompt as the sub-agent's CLAUDE.md
  if (opts.systemPrompt) {
    const groupDir = resolveGroupFolderPath(subFolder);
    fs.mkdirSync(groupDir, { recursive: true });
    fs.writeFileSync(path.join(groupDir, 'CLAUDE.md'), opts.systemPrompt);
  }
  ...
  const virtualGroup: RegisteredGroup = {
    ...
    containerConfig: {
      additionalMounts: opts.mounts.map((m) => ({
        hostPath: m.hostPath,
  ...
  logger.info(
    {
      ...
      subFolder,
      mountCount: opts.mounts.length,
    },
    'Spawning sub-agent container',
  );
  ...
  try {
    await runContainerAgent(
      virtualGroup,
      {
        ...
        isMain: false, // Sub-agents never get main privileges
        assistantName: opts.name,
      },
      (_proc, _containerName) => {
        // Sub-agent process tracking — fire-and-forget for now
      },
      opts.onOutput,
    );
  } catch (err) {
    logger.error(
      { agentId: opts.agentId, name: opts.name, err },
      'Sub-agent container failed',
    );
  }
}
