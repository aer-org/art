# Running AerArt in Docker Sandboxes (Manual Setup)

This guide walks through setting up AerArt inside a [Docker Sandbox](https://docs.docker.com/ai/sandboxes/) from scratch — no install script, no pre-built fork. You'll clone the upstream repo, apply the necessary patches, and have agents running in full hypervisor-level isolation.
...
Enter the sandbox:
```bash
docker sandbox run shell-aer-art-workspace
```
...
## Step 3: Clone and Install AerArt

AerArt must live inside the workspace directory — Docker-in-Docker can only bind-mount from the shared workspace path.
...
mv aer-art "$WORKSPACE/aer-art"
cd "$WORKSPACE/aer-art"
...
AerArt needs several patches to work inside a Docker Sandbox. These handle proxy routing, CA certificates, and Docker-in-Docker mount restrictions.
...
In `src/container-runtime.ts`, the `cleanupOrphans()` function matches containers by the `aer-art-` prefix. Inside a sandbox, the sandbox container itself may match (e.g., `aer-art-docker-sandbox`). Filter out the current hostname:
